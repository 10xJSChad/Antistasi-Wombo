...
TCE_fn_rigVehicle = {
	...
	private _detonateAction  = player addAction [format ["Detonate rigged %1", _vehName], { call TCE_fn_remoteDetonateVehicleBomb; }, [_veh], 5.1, true, true, "", "true", 3];
	_veh setVariable ["TCE_vehicleExplosive", _explosive];
	_veh setVariable ["TCE_vehicleDetonateAction", _detonateAction];

	_veh addEventHandler ["Killed", {
		params ["_unit", "_killer", "_instigator", "_useEffects"];
		
		private __detonateAction  = _unit getVariable "TCE_vehicleDetonateAction";
		player removeAction __detonateAction;
